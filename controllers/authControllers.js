...
export const login = catchAsync(async (req, res, next) => {
    ...
    const user = await User.findOne({ email: email }).select(['+password', '+emailConfirmed']);
    ...
    if (!user.emailConfirmed) {
        return next(
            new AppError('Email not confirmed, try to log in after confirming email.', 401)
        );
    }
...
export const forgotPassword = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json({
            status: 'success',
            message: 'Token sent to email (valid for next 30 minutes)',
        });
    } catch (error) {
        ...
        return next(
            new AppError('There was an error sending email to the user. Try again later!', 500)
        );
    }
});
...
export const updatePassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new AppError('Something went wrong and password not updated. Please try again', 500)
        );
    }
